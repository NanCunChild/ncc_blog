---
title: "Pokapoka搞破坏———溃于蚁穴：小米fastboot溢出分析"
date: 2026-05-14T18:03:20+08:00
draft: false
toc: false
images:
tags:
  ...
  - xiaomi
  - vulnerable
description: "Xiaomi变印度厂了"
---

## 前言
小米存在一个OEM指令 `set-gpu-preemption`，而这个指令存在能溢出到设置Android内核启动参数内存区域的漏洞，实际使用如下：
```
...
# Once commands above done, machine would boot with screen blacked, but ADB is usable
  ...
  if (param_1 == (char *)0x0) {
    ...
    if (param_2 < &DAT_000f4241) {
      ...
      if (param_4 < &DAT_000f4241) {
          ...
          if ((bool)in_ZR) {
            ...
            if (puVar8 <= param_4 && puVar8 <= puVar5) {
              ...
              if (bVar2 && param_1 < pcVar4) {
                FUN_00001efc("/home/work/mnt/miui_cod es1/build_home_rom-odm-merged/kernel_ platform/out/bazel/output_user_root/b197 0bca595d87272e733a0c3ce8a31e/sandbox/ processwrapper-sandbox/142/execroot/_m ain/bootable/bootloader/edk2/MdePkg/Libr ary/BaseLib/SafeString.c"
                             ,0x84b,
                             "InternalSafeStringNoAsciiStrOverlap  (Destination, DestMax, (CHAR8 *)Sou rce, SourceLen + 1)"
                            );
              }
              else {
                ...
                if (((param_3 < param_1 || pcVar1 <= param_ 3) ||
                    (FUN_00001efc("/home/work/mnt/miui_c odes1/build_home_rom-odm-merged/ker nel_platform/out/bazel/output_user_root /b1970bca595d87272e733a0c3ce8a31e/s andbox/processwrapper-sandbox/142/exe croot/_main/bootable/bootloader/edk2/M dePkg/Library/BaseLib/SafeString.c"
                                  ,0x84b,
                                  "InternalSafeStringNoAsciiStrOverl ap (Destination, DestMax, (CHAR8 *)Source, SourceLen + 1)"
        ...
        pcVar4 = 
        "/home/work/mnt/miui_codes1/build_home_ro m-odm-merged/kernel_platform/out/bazel/outp ut_user_root/b1970bca595d87272e733a0c3ce8 a31e/sandbox/processwrapper-sandbox/142/ex ecroot/_main/bootable/bootloader/edk2/MdePk g/Library/BaseLib/SafeString.c"
        ;
        uVar6 = 0x830;
      }
    }
...
fastboot 协议传过来的命令字符串，handler 拿到的 `user_input` 是 `oem set-gpu-preemption` 后面的全部内容，即:

```
value 0 androidboot.selinux=permissive
```
...
```
 msm_kgsl.preempt_enable=value 0 androidboot.selinux=permissive
```
...
**核心 bug:handler 假设 `user_input` 是"一个值"，但实际上它是"命令行剩余的一切"，中间的空格没有被当作分隔符截断。** 把空格之后的东西也一起带进去了。同时函数开头并没有校验此命令调用前是否需要解锁，因此直接执行，在解锁状态下NCC前往圣地的路上一路畅通无阻，一路上没有遇到任何阻碍。
...
`value 0 androidboot.selinux=permissive` 的长度远不止 1，直接被 `if (len == 1)` 否决。即使只传 `fastboot oem set-gpu-preemption 0 androidboot...`，只要后面跟了空格和别的东西,`len` 就不是 1 一样进不去。
